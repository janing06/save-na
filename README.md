...
All data is stored **locally on device** using SQLite via `expo-sqlite`. There is no backend — no accounts, no network requests. TanStack Query wraps all DB calls for caching and invalidation.
